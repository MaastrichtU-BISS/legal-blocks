...
`0.7.0` is on GHCR and public, so an exported platform runs on a machine that
has never seen this repository:

```
ghcr.io/maastrichtu-biss/legal-blocks-composer:0.7.0
ghcr.io/maastrichtu-biss/legal-blocks-platform:0.7.0
ghcr.io/maastrichtu-biss/lawnotation-iaa:0.7.0
```
...
```bash
docker run --rm -p 127.0.0.1:7788:7788 ghcr.io/maastrichtu-biss/legal-blocks-composer:0.7.0
```

Then open <http://localhost:7788>. Its exports name the public `0.7.0` images,
